...
const BtnSaveDirectors = withStyles({
  root: {
    background: "#FF9B0F",
    borderRadius: 50,
    border: 0,
...
export default class SaveDirectorsButton extends React.Component {
  constructor(props) {
    super(props);
    this.saveDirectors = this.saveDirectors.bind(this);
    this.state = {
  ...
  saveDirectors() {
    console.log(
      "persons.getValue = " +
        JSON.stringify(this.props.puDirectorsRef.current.getValue())
    );

    let company_owners = this.props.puDirectorsRef.current.getValue();
    ...
    let saveDirectorsData = {
      company_owners: this.props.puDirectorsRef.current.getValue(),
    ...
    axios
      .post(api_endpoint + "/save_company_owners", saveDirectorsData)
      .then((resp) => {
        ...
        this.props.setDirectorsHandler(resp.data);
      });
  }
  ...
  render() {
    return (
      <div>
        <BtnSaveDirectors onClick={this.saveDirectors}>
          Save directors
        </BtnSaveDirectors>
      </div>
    );
